...
public class ArrayOperation {
    public static void main(String[] args) {
        int[] input = new int[]{2,0,1,2,2,3,0,4,2};
        //ArrayOperation ap = new ArrayOperation();
        //ap.removeElementWithStartAndEnd(input, 2);
    ...
    public int maxSubArray(int[] nums) { 
        int[] local_maxSums = new int[nums.length];
        
        local_maxSums[0] = nums[0];
        int maxSum = local_maxSums[0];
        for(int i = 1; i < nums.length; i++){
            local_maxSums[i] = Math.max(nums[i], nums[i] + local_maxSums[i-1]);
            if(local_maxSums[i] > maxSum) maxSum = local_maxSums[i];
        }
